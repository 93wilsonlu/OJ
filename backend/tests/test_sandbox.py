...
def test_container_uses_private_temp_directory():
    ...
    sandbox._create_container(
        mock_client, "python:3.12-slim", "128m", ["sleep", "1"], sandbox.RUN_TMPFS
    )
...
def test_compile_code_success():
    ...
    # exec_runs:
    # 1. build: cp & compile
    # 2. reset overlay target as root
    # 3. copy /box contents except .tmp to overlay
    mock_container.exec_run.side_effect = [
        (0, b"build ok"),
        (0, b"reset ok"),
        (0, b"copy ok"),
    ...
    assert mock_container.exec_run.call_args_list[1].kwargs["user"] == "0"
    assert "! -name .tmp" in mock_container.exec_run.call_args_list[2].args[0][2]
...
def test_run_custom_input_success():
    ...
    box_dir = {"client": mock_client, "box_archive": b"my_archive"}
    result = sandbox.run_custom_input(1, "python3", 1000, 256, "1 2", box_dir)
    verdict, time_used, mem_used, stdout, stderr, stdout_trunc, stderr_trunc = result
