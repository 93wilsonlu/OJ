...
from app.schemas.submission import JudgeResultOut, SubmissionCreate, SubmissionDetailOut, SubmissionOut
...
async def get_submission(
    ...
    jr_out: JudgeResultOut | None = None
    if judge_result is not None:
        exam = await db.get(Exam, submission.exam_id)
        show_score = exam.show_score if exam else True
        hide = not show_score and current_user.role == "candidate"

        jr_out = JudgeResultOut(
            result_id=judge_result.result_id,
            submission_id=judge_result.submission_id,
            verdict=judge_result.verdict,
            score=float(judge_result.score) if not hide else None,
            passed_count=judge_result.passed_count if not hide else None,
