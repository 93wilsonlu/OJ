...
export default function ExamDetail() {
  ...
  const loadData = async (currentToken: string, id: string) => {
    ...
    try {
      const [examData, problemsData, allProbsData, allUsersData, assignmentsData] = await Promise.all([
        ...
        apiListExamAssignments(currentToken, id) // 撈取原始 assignments
      ]);
      
      setExam(examData);
      setAvailableProblems(allProbsData);
      setAvailableCandidates(allUsersData.items || []);
      setOriginalAssignments(assignmentsData); // 存起來
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const currentAssignments = new Map<string, string>(); // key: "cId_pId", value: assignment_id
      ...
      <div className="bg-oj-surface border border-oj-border rounded-lg p-6 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_350px] gap-8">
          ...
          <div className="border-t md:border-t-0 md:border-l border-oj-border md:pl-6 pt-4 md:pt-0 space-y-6">
             {/* Problems Section */}
             <div>
               <h3 className="text-sm font-semibold text-oj-fg mb-3 flex items-center justify-between">
                 Assigned Problems
                 <span className="bg-oj-surface2 text-oj-fg-muted text-xs px-2 py-0.5 rounded-full">
                    {isEditing ? editProblems.length : assignedProblems.length}
                 </span>
               </h3>
               
               {isEditing ? (
                 <div className="max-h-48 overflow-y-auto bg-oj-bg rounded border border-oj-border p-2 space-y-1">
                   ...
                   {availableProblems.length === 0 && <span className="text-xs text-oj-fg-muted p-1">No problems available.</span>}
                 </div>
               ) : (
                 <>
                     ...
                     <ul className="space-y-2">
                       {assignedProblems.map(p => (
                         <li key={p.problem_id} className="bg-oj-surface2 p-2 rounded border border-oj-border text-sm text-oj-fg flex justify-between items-center">
                           ...
                           <span className={`shrink-0 text-xs px-1.5 py-0.5 rounded capitalize ${p.difficulty === 'easy' ? 'bg-green-900/50 text-green-300' : p.difficulty === 'medium' ? 'bg-yellow-900/50 text-yellow-300' : 'bg-red-900/50 text-red-300'}`}>
                             {p.difficulty}
                           </span>
                         </li>
                       ))}
             ...
             <div>
               ...
               {isEditing ? (
                  <div className="max-h-48 overflow-y-auto bg-oj-bg rounded border border-oj-border p-2 space-y-1">
                    ...
                    {availableCandidates.length === 0 && <span className="text-xs text-oj-fg-muted p-1">No candidates available.</span>}
                  </div>
               ) : (
                 <>
                   {assignedCandidates.length > 0 ? (
                      ...
                      {assignedCandidates.map(c => (
                        <li key={c.user_id} className="bg-oj-surface2 p-2 rounded border border-oj-border text-sm flex flex-col min-w-0">
                          ...
                          <span className="text-oj-fg-muted text-xs font-mono truncate">{c.email}</span>
                        </li>
                      ))}
                    </ul>
                   ) : (
